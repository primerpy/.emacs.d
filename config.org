...
#+end_src
* Python IDE
** Set Environment Python, use python3
#+begin_src emacs-lisp
(setq py-python-command "/home/isaac/anaconda3/bin/python3")
#+end_src
...
** Anaconda Mode
#+begin_src emacs-lisp
(add-hook 'python-mode-hook 'anaconda-mode 'ac-anaconda-setup)
#+end_src
...
#+end_src
* General Setup
** Auto pairing parenthesis etc.
 #+begin_src emacs-lisp
(electric-pair-mode 1)
 #+end_src
** Snippet
I want to load sinppets in orgmode
 #+begin_src emacs-lisp
(require 'yasnippet)
(yas-global-mode 1)
 #+end_src
...
(defun isaac/org-hugo-export ()
  ...
  (save-excursion
    ...
    (let* ((org-pandoc-format 'markdown)
           ...
           (tags (format "[\"%s\"]" (org-element-property :TAGS hl)))
           (categories
            (format "[\"%s\"]"
